...
// function to add comment box on click
    ...
    let currentupVote;  
     ...
     if(!currentupVote.classList.contains('liked')){
      currentupVote.innerText = voteVal +  1
       currentupVote.classList.add('liked')
    }};
    ...
    let voteVal = parseFloat(currentupVote.innerText)  
       if( currentupVote.classList.contains('liked')){
         currentupVote.innerText = voteVal - 1
         currentupVote.classList.remove('liked')
        }};
  ...
  function ShowCommentBox(e){
    ...
    <textarea type="text" class="reply-comment" placeholder="Add a comment..."> </textarea>
  ...
  let  prevReply =
   e.target.parentElement.parentElement.parentElement.parentElement
    ...
    paragraph.innerHTML =   ` 
      ...
      <textarea type="text" class="reply-comment edit" > ${text.trim()} </textarea>
 ...
 let input = prevReply.querySelector('.reply-comment')
    ...
    prevReply.addEventListener('click' , (e)=>{
      // save to the array and local storage 
       ...
       if(comments[arrIndex].replies[currReplyIndex]){
